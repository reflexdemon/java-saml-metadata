package io.vpv.saml.metasaml.metadata.service;
...
import io.vpv.saml.metasaml.metadata.settings.Saml2Settings;
import io.vpv.saml.metasaml.metadata.settings.SettingsBuilder;
import io.vpv.saml.metasaml.metadata.util.Constants;
...
/**
 * IdPMetadataParser class of VPV Saml Tool
 *
...
public class IdPMetadataParser {
	...
	public static void main(String[] args) throws Exception {
		...
		//UAT
//		String endpoint = "https://login.microsoftonline.com/042a40a1-b128-4ac4-8648-016ffa121487/federationmetadata/2007-06/federationmetadata.xml?appid=2b654af3-d322-4434-95dc-d47eeacf1db1";
		//Prod
		String endpoint = "https://login.microsoftonline.com/042a40a1-b128-4ac4-8648-016ffa121487/federationmetadata/2007-06/federationmetadata.xml?appid=35921e5c-2c4b-4bda-8207-53593fdc754b";
				
		
		URL urlEndpoint = new URL(endpoint);
		Map<String, Object> idpInfo = IdPMetadataParser.parseRemoteXML(urlEndpoint);
//		Map<String, Object> idpInfo = IdPMetadataParser.parseFileXML("saml/uat-meta.xml");
		idpInfo.keySet().stream().map(key -> key + " ->\n" + idpInfo.get(key) + "\n######")
				.forEach(System.out::println);
//		String jsonFromProperties3 = new PropertiesToJsonConverter().convertFromValuesAsObjectMap(idpInfo);
//		System.out.println(jsonFromProperties3);
	}

	/**
	 * Get IdP Metadata Info from XML Document
	 * 
	 * @param xmlDocument         XML document hat contains IdP metadata
	 * @param entityId            Entity Id of the desired IdP, if no entity Id is
	 *                            provided and the XML metadata contains more than
	 *                            one IDPSSODescriptor, the first is returned
	 * @param desiredNameIdFormat If available on IdP metadata, use that
	 *                            nameIdFormat
	 * @param desiredSSOBinding   Parse specific binding SSO endpoint.
	 * @param desiredSLOBinding   Parse specific binding SLO endpoint.
	 * 
	 * @return Mapped values with metadata info in Saml2Settings format
	 * @throws XPathExpressionException
	 */
	public static Map<String, Object> parseXML(Document xmlDocument, String entityId, String desiredNameIdFormat,
			String desiredSSOBinding, String desiredSLOBinding) throws XPathException {
		...
		try {
			...
			if (idpDescriptorNodes.getLength() > 0) {
				...
				NodeList ssoNodes = Util.query(xmlDocument,
						"./md:SingleSignOnService[@Binding=\"" + desiredSSOBinding + "\"]", idpDescriptorNode);
				...
				NodeList sloNodes = Util.query(xmlDocument,
						"./md:SingleLogoutService[@Binding=\"" + desiredSLOBinding + "\"]", idpDescriptorNode);
					...
					if (keyDescriptorCertSigningNodes.getLength() > 0) {
						...
						for (int i = 0; i < keyDescriptorCertSigningNodes.getLength(); i++) {
							...
							if (i == 0 && !hasEncryptionCert) {
								...
								metadataInfo.put(SettingsBuilder.IDP_X509CERTMULTI_PROPERTY_KEY + "." + (index++),
										signingCert);
							}
						}
					}
				...
				for (int i = 0; i < nameIdFormatNodes.getLength(); i++) {
					...
					if (nameIdFormat != null
							&& (desiredNameIdFormat == null || desiredNameIdFormat.equals(nameIdFormat))) {
	 ...
	 * Get IdP Metadata Info from XML Document
	 * 
	 * @param xmlDocument XML document that contains IdP metadata
	 * @param entityId    Entity Id of the desired IdP, if no entity Id is provided
	 *                    and the XML metadata contains more than one
	 ...
	 * 
	 * @param xmlFileName         Filename of the XML filename that contains IdP
	 *                            metadata
	 * @param entityId            Entity Id of the desired IdP, if no entity Id is
	 *                            provided and the XML metadata contains more than
	 *                            one IDPSSODescriptor, the first is returned
	 ...
	 * Get IdP Metadata Info from XML file
	 * 
	 * @param xmlFileName Filename of the XML filename that contains IdP metadata
	 * @param entityId    Entity Id of the desired IdP, if no entity Id is provided
	 *                    and the XML metadata contains more than one
	 *                    IDPSSODescriptor, the first is returned
	 * 
	...
	public static Map<String, Object> parseFileXML(String xmlFileName, String entityId) throws Exception {
		return parseFileXML(xmlFileName, entityId, null, Constants.BINDING_HTTP_REDIRECT,
	 ...
	 * Get IdP Metadata Info from XML file
	 * 
	 * @param xmlURL              URL to the XML document that contains IdP metadata
	 * @param entityId            Entity Id of the desired IdP, if no entity Id is
	 *                            provided and the XML metadata contains more than
	 *                            one IDPSSODescriptor, the first is returned
	 * @param desiredNameIdFormat If available on IdP metadata, use that
	 ...
	 * @throws Exception
	 */
	public static Map<String, Object> parseRemoteXML(URL xmlURL, String entityId, String desiredNameIdFormat,
			String desiredSSOBinding, String desiredSLOBinding) throws Exception {
	 ...
	 * 
	 * @param xmlURL   URL to the XML document that contains IdP metadata
	 * @param entityId Entity Id of the desired IdP, if no entity Id is provided and
	 *                 the XML metadata contains more than one IDPSSODescriptor, the
	 *                 first is returned
	 * 
	 * @return Mapped values with metadata info in Saml2Settings format
	 * @throws Exception
	 ...
	 *
	 * @param settings     the Saml2Settings object
	 * @param metadataInfo mapped values with metadata info in Saml2Settings format
	 * 
